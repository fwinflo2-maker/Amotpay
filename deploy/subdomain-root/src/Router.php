...
final class Router
{
    public function dispatch(Request $request): void
    {
        ...
        try {
            match (true) {
                ...
                $method === 'POST' && $path === '/api/webhooks/cashramp' => $this->cashrampWebhook($cryptoService, $request, $settingsSvc),

                $method === 'GET' && $path === '/api/callbacks/magma/success' => $this->magmaCallback('success'),
                $method === 'GET' && $path === '/api/callbacks/magma/error' => $this->magmaCallback('error'),
                ...
                $method === 'PUT' && $path === '/api/admin/providers' => $this->adminSaveProviders($adminMw, $settingsSvc, $request, $magma, $cashramp),
                $method === 'GET' && $path === '/api/admin/magma-setup' => $this->adminMagmaSetup($adminMw, $settingsSvc, $request),
                $method === 'GET' && $path === '/api/admin/health-check' => $this->adminHealthCheck($adminMw, $request, $magma, $cashramp),
    ...
    private function cashrampWebhook(CryptoService $svc, Request $request, SettingsService $settings): void
    {
        $token = $settings->get('CASHRAMP_WEBHOOK_TOKEN') ?? Env::get('CASHRAMP_WEBHOOK_TOKEN', '') ?? '';
    ...
    private function magmaCallback(string $type): void
    {
        Response::json([
            'success' => true,
            'message' => 'Magma callback received',
            'type' => $type,
        ]);
    }
    ...
    private function adminSaveProviders(
        ...
        Response::json([
            ...
            'data' => [
                'providers' => $settings->getMaskedForAdmin(),
                'health' => [
                    'magma' => (new MagmaService())->healthCheck(),
                    'cashramp' => (new CashrampService())->healthCheck(),
                ],
            ],
        ]);
    }
